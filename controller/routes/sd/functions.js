...
const getOfferValue = async (
    ...
    if (offerName)
        query = await pool.query(
            'SELECT value FROM offers WHERE service_id = $1 AND name = $2',
            [serviceID, offerName]
        )
    else
        query = await pool.query(
            'SELECT value FROM offers WHERE offer_id = $1',
            [offerID]
        )
    return query.rows[0].value
}

const updateApplicationPhase1 = async (
    client,
    statusChange,
    salesRepDetails,
    appID
    ...
    try {
        await client.query(
            'UPDATE sales_applications_details SET sales_rep_details = $1, status_change_date = CURRENT_TIMESTAMP WHERE id = $2',
            [salesRepDetails, appID]
        )
        await client.query(
            'UPDATE sales_applications SET status = $1 WHERE id = $2',
            [statusChange, appID]
        )
        return { done: true }
    } catch (err) {
...
const updateApplicationPhase2 = async (
    client,
    statusChange,
    salesRepDetails,
    appID
    ...
    try {
        await client.query(
            'UPDATE sales_applications_details SET final_sales_rep_details = $1 WHERE id = $2',
            [salesRepDetails, appID]
        )
        await client.query(
            'UPDATE sales_applications SET status = $1, last_change_date = CURRENT_TIMESTAMP WHERE id = $2',
            [statusChange, appID]
        )
        ...
        if (statusChange === 'approved') {
            ...
            const getSubmitter = await client.query(
                'SELECT submitter FROM sales_applications WHERE id = $1',
                [appID]
            )
            ...
            const transactionID =
                insertTransactionAndReturnID.rows[0].transaction_id
            await client.query(
                'UPDATE sales_applications_details SET transaction_id = $1 WHERE id = $2',
                [transactionID, appID]
            )
            ...
            if (checkGoalSuccessQuery.rowCount === 1) {
                if (
                    checkGoalSuccessQuery.rows[0].done ===
                    checkGoalSuccessQuery.rows[0].goal
                )
                    await client.query(
                        'UPDATE goals SET success = true, done = done + 1 WHERE goal_id = $1',
                        [checkGoalSuccessQuery.rows[0].goal_id]
                    )
                else {
                    await client.query(
                        'UPDATE goals SET done = done + 1 WHERE goal_id = $1',
                        [checkGoalSuccessQuery.rows[0].goal_id]
                    )
                }
            }
...
const verifySdResponsibleForUser = async (userID, sdID) => {
    verifyQueryStatement =
        'SELECT user_id FROM login WHERE assigned_to = (SELECT responsible_area FROM login WHERE user_id = $1)'
    const query = await pool.query(verifyQueryStatement, [sdID])
    if (query.rows[0].user_id !== userID) return false
    return true
}
