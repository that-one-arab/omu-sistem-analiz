...
app.put(
    ...
    async (req, res) => {
        ...
        const serverErrorStr =
            'server error occurred while attempting to update application'
        const unexpectedStatusChangePhase1 =
            "Expected 'processing' or 'rejected' instead got '" +
            statusChange +
            "' AT " +
            __dirname
        const unexpectedStatusChangePhase2 =
            "Expected 'approved' or 'rejected' instead got '" +
            statusChange +
            "' AT " +
            __dirname
        ...
        try {
            ...
            if (currentStatus === 'sent') {
                ...
                // IF rejected, jump to phase2 since no further updates will be allowed after 'rejected' status
                    ...
                    result = await updateApplicationPhase2(
                        client,
                        statusChange,
                        salesRepDetails,
                        appID
                ...
                // ELSE
                    ...
                    result = await updateApplicationPhase1(
                        client,
                        statusChange,
                        salesRepDetails,
                        appID
            ...
            if (result.done) {
                ...
                return res
                    .status(200)
                    .json('Application was updated successfully')
                // ELSE
            } else {
    ...
    if (userID) {
        ...
        if (!(await verifySdResponsibleForUser(userID, sdID))) {
            const errorStr =
                "user ID '" +
                userInfo.userID +
                "' attempted to access /goal/sd and fetch specific user ID '" +
                req.query.userID +
                "'s goal but did not have permission because the user does not fall under their responsibility at " +
                __dirname
            return customStatusError(
                errorStr,
                res,
                403,
                'you are not responsible for this user'
            )
        }
