...
const verifyUpdateApplication = async (req, res, next) => {
    ...
    if (userRole !== 'sales_assistant' && userRole !== 'sales_assistant_chef')
        return customStatusError(
            'submitted request does not have SD or SDC role',
            res,
            401,
            'You are not authorized to update applications'
        )

    const { applicationID } = req.params
    // if application does not exist in database
    verifyAppIDQuery = await pool.query(
        'SELECT id, status, activator FROM sales_applications WHERE id = $1',
        [applicationID]
    )
    ...
    if (
        verifyAppIDQuery.rows[0]?.status === 'approved' ||
        verifyAppIDQuery.rows[0]?.status === 'rejected'
    )
        return customStatusError(
            "requested application with ID '" +
                applicationID +
                "' is approved or rejected, can not make further changes at " +
                __dirname,
            res,
            401,
    ...
    if (!expectedStatusChange.includes(req.body.statusChange))
        return customStatusError(
            "requested application with ID '" +
                applicationID +
                "' has an unexpected req.body.statusChange field input at " +
                __dirname,
            res,
            401,
            'Unexpected input'
    ...
    if (verifyAppIDQuery.rows[0]?.activator !== name)
        return customStatusError(
            "application ID '" +
                applicationID +
                "' with activator '" +
                verifyAppIDQuery.rows[0]?.activator +
                "' tried to be updated by user '" +
                name +
                "'  at " +
                __dirname,
            res,
