...
app.get('/services', authenticateToken, async (req, res) => {
    try {
        ...
        if (active) serviceStatement = selectedStatement + activeCondition
        ...
        return res
            .status(500)
            .json('an error occurred while fetching services data')
    }
})
...
app.get('/service/:serviceID', async (req, res) => {
    try {
        ...
        if (active) offersStatement = selectedStatement + activeCondition

        const offersQuery = await pool.query(offersStatement, [serviceID])
        if (offersQuery.rowCount === 0) {
            const errorStr =
                'offers under requested service ID ' +
                serviceID +
            ...
            return customStatusError(
                ...
                res,
                406,
                'offers under your requested service ID does not exist'
            )
        }
...
app.get('/application/:applicationID', authenticateToken, async (req, res) => {
    try {
        ...
        const customErr =
            'at application/:applicationID, database query returned an empty array'
        ...
        // else, return the application ONLY if the request submitter is the same as the application submitter
            ...
            query = await pool.query(
                queryStatement + submitterConditionalStatement,
                [applicationID, res.locals.userInfo.userID]
            )
            ...
            if (query.rows.length === 0)
                return customStatusError(
                    ...
                    res,
                    403,
                    'You are not authorized to fetch this application'
                )
        }
        ...
        return status500Error(
            err,
            res,
            'server error could not fetch specific application'
        )
    }
})
...
app.get('/applications/:query', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        if (month || year || day) {
            ...
            selectQuery = await getDealerApplications(
                query,
                dayObj,
                userID,
                status,
        ...
        return res
            .status(500)
            .json('server error, Unable to fetch dealer applications')
    }
})
...
app.get('/user', authenticateToken, async (req, res) => {
    try {
        ...
        const selectStatement =
            'SELECT role, balance, register_date, email, name FROM login WHERE user_id = $1'
        ...
        return status500Error(
            err,
            res,
            'server error, Could not fetch your information'
        )
    }
})
    ...
    async (req, res) => {
        try {
            ...
            const updateStatement =
                'UPDATE login SET hash = $1 WHERE user_id = $2'
            ...
            return status500Error(
                err,
                res,
                'server error, unable to change your password'
            )
        }
    }
)
...
app.patch(
    ...
    async (req, res) => {
        try {
            ...
            const { name } = req.body
            const updateStatement =
                'UPDATE login SET name = $1 WHERE user_id = $2'
            ...
            return status500Error(
                err,
                res,
                'server error, unable to change your name'
            )
        }
    }
)
...
app.get('/user/:requestedUserID', authenticateToken, async (req, res) => {
    ...
    if (role === 'sales_assistant')
        ...
        return customStatusError(
            "user ID'" +
                requesterID +
                "' attempted to access /users but did not have 'sales_assistant' role",
            res,
            403,
...
app.get('/users', authenticateToken, async (req, res) => {
    ...
    if (role === 'sales_assistant') await getSdUsers(name, res)
    else if (role === 'sales_assistant_chef') await getSdcUsers(res)
    else
        return customStatusError(
            "user '" +
                name +
...
app.get('/goal', authenticateToken, async (req, res) => {
    try {
        ...
        return status500Error(
            err,
            res,
            'server error, could not fetch your goal'
        )
    }
})
...
app.get('/transactions', authenticateToken, async (req, res) => {
    ...
    try {
        if (date) {
            const queryStatement =
                "SELECT * FROM transactions WHERE EXTRACT(month from date) = (SELECT date_part('month', $1 ::timestamp)) AND EXTRACT(year from date) = (SELECT date_part('year', $1 ::timestamp)) AND user_id = $2"
...
app.get('/transactions/:userID', authenticateToken, async (req, res) => {
    ...
    if (role === 'sales_assistant')
        return customStatusError(
            ...
            res,
            401,
            'You are not authorized to make this query'
        )
    ...
    try {
        ...
        return status500Error(
            error,
            res,
            "Could not fetch this user's transactions"
        )
    }
})
...
app.get('/transaction/:appID', async (req, res) => {
    ...
    try {
        if (role === 'dealer') {
            const { userID } = userInfo
            const queryStatement =
                'SELECT * FROM transactions WHERE user_id = $1 AND app_id = $2'
            ...
            const queryStatement =
                'SELECT * FROM transactions WHERE app_id = $1'
        ...
        return status500Error(
            error,
            res,
            "Could not fetch that specific application's transaction"
        )
    }
})
...
app.get('/report/transactions', authenticateToken, async (req, res) => {
    ...
    const queryStatement =
        'SELECT * FROM transaction_reports WHERE user_id = $1'
    const queryStatementDate =
        "SELECT * FROM transaction_reports WHERE user_id = $1 AND EXTRACT(month from date) = (SELECT date_part('month', $2 ::timestamp)) AND EXTRACT(year from date) = (SELECT date_part('year', $2 ::timestamp))"
    const queryStatementAppTransacForReport =
        "SELECT sales_applications_details.client_name, sales_applications_details.selected_service, transactions.amount, transactions.app_id, transactions.transaction_id, transactions.balance_before, transactions.balance_after, to_char(transactions.date, 'YYYY-MM-DD') AS date, transactions.report_id FROM sales_applications_details INNER JOIN transactions ON sales_applications_details.transaction_id = transactions.transaction_id WHERE transactions.report_id = $1 AND transactions.user_id = $2"
    let query

    try {
        if (date)
            query = await pool.query(queryStatementDate, [
                userInfo.userID,
                date,
            ])
        else if (reportID)
            query = await pool.query(queryStatementAppTransacForReport, [
                reportID,
                userInfo.userID,
            ])
        else query = await pool.query(queryStatement, [userInfo.userID])
        return res.json(query.rows)
    } catch (error) {
...
app.get('/report/transactions/count', authenticateToken, async (req, res) => {
    ...
    const { date } = req.query
    const queryStatement =
        'SELECT COUNT(report_id) FROM transaction_reports WHERE user_id = $1'
    const queryStatementDate =
        "SELECT COUNT(report_id) FROM transaction_reports WHERE user_id = $1 AND EXTRACT(month from date) = (SELECT date_part('month', $2 ::timestamp)) AND EXTRACT(year from date) = (SELECT date_part('year', $2 ::timestamp))"
    let query
    try {
        if (date)
            query = await pool.query(queryStatementDate, [
        ...
        if (query.rows[0].count) return res.json(query.rows[0])
...
app.get('/report/transactions/:userID', authenticateToken, async (req, res) => {
    ...
    if (
        userInfo.role !== 'sales_assistant_chef' &&
        userInfo.role !== 'sales_assistant'
    )
        return customStatusError(
            "user role: '" + userInfo.role + "' is not authenticated",
            res,
            401,
            "You're not authenticated to make this request"
        )
    const { userID } = req.params
    const { date } = req.query
    try {
