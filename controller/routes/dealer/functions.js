...
const getSaleActivator = async (dealerID, res) => {
    const queryStatment =
        "SELECT name, user_id FROM login WHERE role = 'sales_assistant' AND assigned_area = (SELECT assigned_area FROM login WHERE user_id = $1)"
    try {
        ...
        return status500Error(
            error,
            res,
            "Could not fetch your sales' activator"
        )
    }
}
...
const verifyApplicationInput = async (reqBody) => {
    const { selectedService, selectedOffer, clientDescription, clientName } =
        reqBody
    try {
...
const sendApplication = async (userInfo, reqBody, photoURLS, res) => {
    ...
    try {
        ...
        const { name: saleActivator, id: saleActivatorId } = await getSaleActivator(userID, res)
        // insert into sales_applications and return id
        const query = await client.query(
            'INSERT INTO sales_applications(submitter, submit_time, activator, activator_id, client_name, status) VALUES($1, $2, $3, $4, $5, $6) RETURNING id',
            [userID, 'NOW()', saleActivator, saleActivatorId, clientName, 'sent']
        )
        // insert into sales_applications_details
        await client.query(
            'INSERT INTO sales_applications_details(client_name, selected_service, selected_offer, description, id, image_urls) VALUES($1, $2, $3, $4, $5, $6)',
            [
                clientName,
                selectedService,
                selectedOffer,
