...
const uniqid = require('uniqid')(
    ...
    async () => {
        ...
        try {
            ...
            const getDealersQuery = await client.query(
                "SELECT user_id FROM login WHERE role = 'dealer'"
            )
            ...
            const transactionStatement =
                "SELECT SUM(amount) FROM transactions WHERE EXTRACT(month from date) = (SELECT date_part('month', (SELECT date_trunc('day', NOW() - interval '1 month')))) AND EXTRACT(year from date) = (SELECT date_part('year', (SELECT date_trunc('day', NOW() - interval '1 month')))) AND user_id = $1"
            // get current date
            const date = new Date()
            ...
            for (let i = 0; i < dealers.length; i++) {
                ...
                if (transactionSum !== null) {
                    ...
                    const insertAndReturnReportID = await client.query(
                        'INSERT INTO transaction_reports VALUES ($1, $2, $3) RETURNING report_id',
                        [dealers[i], transactionSum, prevMonthDate]
                    )
                    ...
                    // update statement
                    const updateTransactionsStatement =
                        "UPDATE transactions SET report_id = $1 WHERE EXTRACT(month from date) = (SELECT date_part('month', (SELECT date_trunc('day', NOW() - interval '1 month')))) AND EXTRACT(year from date) = (SELECT date_part('year', (SELECT date_trunc('day', NOW() - interval '1 month')))) AND user_id = $2"
                    // perform the update
                    await client.query(updateTransactionsStatement, [
                        reportID,
                        dealers[i],
                    ])
            ...
            const emailData = {
                ...
                text:
                    "A server error occurred while printing monthly transaction report, the error's content is \n " +
                    errorID +
                    ': ' +
                    error,
            }
            mailgun.messages().send(emailData, (err, body) => {
                ...
                console.log(
                    'successfuly sent an email to mohammad.nadom98@gmail.com, body: ',
                    body
                )
                process.exit(0)
            })
